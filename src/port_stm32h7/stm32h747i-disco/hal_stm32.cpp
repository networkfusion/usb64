...
void n64hal_gpio_init()
{
    ...
    for (int i = 0; i < USB64_PIN_MAX; i++)
    {
        ...
        case N64_CONSOLE_SENSE_PIN:
            _dev_gpio[i].pin = GPIO_PIN_8; //D3
            _dev_gpio[i].port = GPIOF;
            break;
        case N64_CONTROLLER_1_PIN:
            _dev_gpio[i].pin = GPIO_PIN_3; //D2
            _dev_gpio[i].port = GPIOJ;
            break;
        case N64_CONTROLLER_2_PIN:
            _dev_gpio[i].pin = GPIO_PIN_4; //D4
            _dev_gpio[i].port = GPIOJ;
            break;
        case N64_CONTROLLER_3_PIN:
            _dev_gpio[i].pin = GPIO_PIN_0; //D7
            _dev_gpio[i].port = GPIOJ;
            break;
        case N64_CONTROLLER_4_PIN:
            _dev_gpio[i].pin = GPIO_PIN_5; //D8
            _dev_gpio[i].port = GPIOJ;
            break;
        }
    }
...
void n64hal_input_swap(usb64_pin_t pin, uint8_t val)
{
    ...
    temp &= ~(GPIO_MODER_MODE10 << (offset * 2U)); //TODO: FIXME - not sure if this is correct! (Was GPIO_MODER_MODER0)
    temp |= ((mode & GPIO_MODER_MODE10) << (offset * 2U)); //TODO: FIXME - not sure if this is correct! (Was GPIO_MODER_MODER0)
...
//This is a shared interrupt line for player 1 and 2. Need to determine which pin caused it and call appropriately
